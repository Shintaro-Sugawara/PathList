...
- **Snapdragon（Arm系CPU）での x64 エミュレーション環境**でも動作するよう設計していますが、開発者は Arm系PCを所持していないため、テストは行っていません。  
   - Arm系 CPU 環境でのExcel 出力機能は、動作保証対象外です。  
- Arm系PC での Excel 出力機能のサポートは、今後も予定していません。
   - Arm版Excel と x64版Excel の両方を用意する必要があるため。
...
1. 【初回のみ】スタートメニューから `Path List Shortcut` または `Path List Pro Shortcut` を実行して、デスクトップにショートカットを作成してください。  
...
| 1.01       | 2025-11-14 | Microsoft Store への登録対応としての更新                               |
| 1.02       | 2025-11-19 | 1. ショートカット作成機能を追加（通常のショートカット作成操作では、ドロップフォルダのパスをアプリに渡せないケースがあります） <br> 2. SnapDragon（Arm系CPU）での x86エミュレーション環境でもフォルダ内容を取得できるよう再設計 |
...
|------------|------------|----------------------------------------------------------------------|
| 1.00       | 2025-11-08 | 初期リリース                                                          |
| 1.01       | 2025-11-19 | Ver.1.02 はフリー版とのバージョン番号調整のためリリースをスキップ     |
| 1.02       | 2025-11-19 | 1. ショートカット作成機能を追加（通常のショートカット作成操作では、ドロップフォルダのパスをアプリに渡せないケースがあります） <br> 2. SnapDragon（Arm系CPU）での x86エミュレーション環境でもフォルダ内容を取得できるよう再設計 |
...
[PathList_1.03.py を GitHub で見る](https://github.com/Shintaro-Sugawara/PathList/blob/master/src/PathList_1.03.py)
